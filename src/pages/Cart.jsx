...
//       (item) =>
//         `▪️ ${item.name} (${item.weight}) ×${item.qty} — €${(item.price * item.qty).toFixed(2)}`
//     );

//     const messageParts = [
//       "🥩 *NEW ORDER - MrButcher NL Website*",
//       "",
...
//           {cartItems.map((item) => (
//             // key uses both name and weight so same product different weight = different row
//             <div className="cart-item" key={`${item.name}-${item.weight}`}>
...
//                   {/* ✅ Pass both name AND weight */}
//                   <button onClick={() => updateQty(item.name, item.weight, item.qty - 1)}>−</button>
//                   <span>{item.qty}</span>
//                   <button onClick={() => updateQty(item.name, item.weight, item.qty + 1)}>+</button>
//                 </div>
...
//             {cartItems.map((item) => (
//               <div className="summary-line" key={`${item.name}-${item.weight}`}>
...
const Cart = () => {
  ...
  const validate = () => {
    ...
    if (!customerName.trim()) e.name = "Please enter your name.";
    if (!customerPhone.trim()) e.phone = "Please enter your phone number.";
    if (!customerCity.trim()) e.city = "Please enter your city.";
    return e;
  };
  ...
  const handleOrder = () => {
    ...
    if (customerEmail.trim()) {
      messageParts.push(`📧 *Email:* ${customerEmail}`);
    }
        ...
        <div className="cart-summary">
          ...
          <div className="checkout-form">
            ...
            <div className="form-group">
              ...
              <input
                type="text"
                placeholder="e.g. Amsterdam / Groningen / Rotterdam"
                value={customerCity}
                onChange={(e) => {
            ...
            <div className="form-group">
              ...
              <input
                type="email"
                placeholder="e.g. jan.devries@gmail.com"
                value={customerEmail}
